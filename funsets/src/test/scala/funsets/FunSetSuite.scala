...
class FunSetSuite extends FunSuite {
  ...
  test("filter filters the set successfully") {
    new TestSets {
      val s = filter(evenNumbers, (x => x > 0 && x < 6))
      assert(contains(s, 4), "Filter 4")
      assert(!contains(s, 6), "Filter 6")
    }
  }

  test("forall works successfully") {
    new TestSets {
  ...
  test("exists works successfully") {
    new TestSets {
